      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <section style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 clamp(20px,5vw,100px)', maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ opacity: mounted ? 1 : 0, transform: mounted ? 'none' : 'translateY(20px)', transition: 'all 0.7s ease' }}>
            <div style={{ marginBottom: 24 }}><SubjectIcon subject="computer-science" color={COLOR} size={80} /></div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)', fontFamily: 'Inter,sans-serif', marginBottom: 14 }}>GCSE · OCR J277</div>
            <h1 style={{ fontSize: 'clamp(44px,8vw,88px)', fontWeight: 300, lineHeight: 1.05, letterSpacing: '-0.03em', color: '#fff', marginBottom: 20 }}>
              Computer<br />
              <span style={{ fontWeight: 700, background: `linear-gradient(135deg,#fff 40%,${COLOR} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Science.</span>
            </h1>
            <p style={{ fontFamily: 'Inter,sans-serif', fontSize: 16, color: 'rgba(255,255,255,0.4)', maxWidth: 480, lineHeight: 1.75, marginBottom: 36 }}>
              Algorithms, logic gates, programming, networking — the full OCR J277 specification.
            </p>
            {stats && (
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 clamp(20px,5vw,100px) 40px' }}>
          ...
          <PageValueSection
            title="Revising Computer Science Effectively"
            summary="Computer Science rewards precision. Short, frequent rounds that test definitions and logic patterns outperform long passive re-reading."
            points={[
              { label: 'OCR J277 aligned', text: 'All content maps to the official specification topics and command words.' },
              { label: 'Logic and algorithm focus', text: 'Practice trace tables and pseudocode patterns until they become automatic.' },
              { label: 'Theory and applied', text: 'Questions combine knowledge recall with applied thinking scenarios.' },
            ]}
            checklist={['Complete one category per session.', 'Draw one trace table or flowchart from memory.', 'Re-test weak logic topics the next day.']}
          />
        </div>
      </div>
